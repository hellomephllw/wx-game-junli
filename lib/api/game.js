...
router.post('/game', upload.single('background'), function(req, res) {
    ...
    var imgPath = req.file ? '/images/upload/' + req.file.filename : false;
    ...
    if (imgPath) {
        ...
        params = [name, imgPath, desc];
    } else {
        sql = 'insert into Game values(null, ?, ?, ?)';
        params = [name, '/images/bg/empty.jpg', desc];
    }
...
router.patch('/game', upload.single('background'), function(req, res) {
    ...
    var params = [];
    if (imgPath) {
        sql = 'update Game set name=?,imgPath=?,description=? where id=?';
        params = [name, imgPath, desc, id];
    } else {
